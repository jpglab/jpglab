...
We are now going to attempt **PHASE 4** of the refactor.
...
Our goals at this phase of the refactor (**PHASE 4**):

- [x] Create `GenericPTPCamera` implementation
- [x] Implement `SonyPropertyMapper`
- [x] Refactor `SonyCamera` to extend `GenericPTPCamera` including the handshake/authentication process
- [x] Add property mapping tests
- [x] Get basic connections for Sony camera test working
- [ ] Get all functions for Sony camera test working (we had some issues last time, they're commented out)

Success criteria for this phase of the refactor (**PHASE 4**):

- [x] `src_new/tests/generic_camera.ts`:
    - [x] tests using Vitest
    - [x] open and close a session with the camera over USB transport
- [ ] `src_new/tests/sony_camera.ts`:
    - [x] tests using Vitest
    - [x] open and close a session with the camera over USB transport
    - [x] run through the handshake/authentication process to get to a PTP Remote Protocol Connected state
    - [ ] supports all the functions we want
        - [ ] get current ISO
        ...
        - [ ] save captured photo
        - [ ] enable liveview
        - [ ] retrieve live view image
        - [ ] set OSD mode
        - [ ] retrieve OSD image
- [ ] `bun run test:generic_camera` and `bun run test:sony_camera` run the new scripts using Vitest
