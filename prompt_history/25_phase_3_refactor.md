...
We are now going to attempt **PHASE 3** of the refactor.
...
Our goals at this phase of the refactor (**PHASE 3**):
...
Success criteria for this phase of the refactor (**PHASE 3**):

- [ ] `src_new/tests/ptp.ts`:
    - [ ] tests using Vitest
    - [ ] we can instantiate the new `USBTransport`, detect a camera, claim the interface (same behavior as `src_new/tests/usb.ts`, although we need way less verbose logging)
    - [ ] we can instantiate the new `PTPProtocol` and inject the transport
    - [ ] we can build a message to open a new session
    - [ ] the session opens successfully
- [ ] `bun run test:ptp` runs the new script using Vitest
