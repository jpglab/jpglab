...
We are now going to attempt **PHASE 1** of the refactor.
...
Our goals at this phase of the refactor (**PHASE 1**):
...
Success criteria for this phase of the refactor (**PHASE 1**):

- [ ] A new TSConfig
- [ ] All interfaces complete
- [ ] Clean scripts that apply to all the code in the new `src_new` folder:
    - [ ] `bun run compile`
    - [ ] `bun run lint`
    - [ ] `bun run format`
- [ ] A script to do all three (`bun run all`)
- [ ] `bun run all` works with exit code 0 and no errors/warnings
- [ ] Path aliases for everything (e.g. `@core/`, `@transport/`, `@factories/`, `@camera/`, `@application/`)
