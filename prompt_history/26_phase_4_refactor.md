...
- [ ] Refactor `SonyCamera` to extend `GenericPTPCamera` including the handshake/authentication process
- [ ] Add property mapping tests

Success criteria for this phase of the refactor (**PHASE 4**):

- [ ] `src_new/tests/generic_camera.ts`:
    - [ ] tests using Vitest
    - [ ] open and close a session with the camera over USB transport
- [ ] `src_new/tests/sony_camera.ts`:
    - [ ] tests using Vitest
    - [ ] open and close a session with the camera over USB transport
    - [ ] run through the handshake/authentication process to get to a PTP Remote Protocol Connected state
