...
Our migration is complete. I'm not super happy with how the ergonomics of the application layer work currently. I think it could be easier.

Read through the `src/tests/05-camera.ts` file for a usage example of the high-level API. Here are some critiques:

- What the hell is an application factory? The name is extremely generic.
- Does the user need to know that any of these objects are in fact a factory? Does that assist in usability?
- The user needs to import a device finder which is specific to USB, versus a generic device finder that is agnostic of the transport layer.
- Each of these is discrete in nature.
- 8 steps to get to a connected camera:
    - Import DeviceFinder
    - Import ApplicationFactory
    - Import Camera
    - Instantiate DeviceFinder
    - Call FindDevices
    - Instantiate new CameraFactory
    - Detect Vendor
    - Create Camera

Can you propose something that is much easier for users?
